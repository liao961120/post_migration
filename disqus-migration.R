...
old_posts <- gsub("/$", ".html", new_posts) %>% paste0(baseURL, .)
new_posts <- paste0(baseURL, new_posts)
...
old_custom <- c('/2018/01/31/RlearningPath.html', 
  ...
  paste0(baseURL, .)
new_custom <- tolower(old_custom) %>% gsub(".html", "/", ., fixed = T)
old_posts <- c(old_posts, old_custom)
new_posts <- c(new_posts, new_custom)
